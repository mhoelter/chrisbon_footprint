...
def extract_data_from_json(json_file_path: str) -> List[Dict[str, Any]]:
    ...
    for obj in data.get('timelineObjects', []):
        if 'activitySegment' in obj:
            ...
            # Create a unique identifier for this activity segment
            # This will be used to group waypointPath points with their parent
            segment_id = f"segment_{len(results)}"
            ...
            for waypoints in waypointPath:
                ...
                results.append({
                    ...
                    'End Add.': '', 
                    'End Name': '',
                    # Use the parent activity segment's start time for the waypointPath points
                    'Start': startTime,
                    'End': '',
                    'Activity Dist.': '',
                    'Activity': travelMode,
                    ...
                    'Visit Conf.': '', 
                    'Place Loc. Conf.': '',
                    'Segment ID': segment_id  # Link to parent segment
                })
            ...
            results.append({
                ...
                'Segment ID': ''  # No segment ID for place visits
            })
...
def write_to_csv(data: List[Dict[str, Any]], csv_file_path: str):
    ...
    # Get headers from the first item, excluding the Segment ID which is used only for sorting
    headers = [key for key in data[0].keys() if key != 'Segment ID']
...
def main():
    ...
    json_dir = 'JSON'
    output_csv = 'Combined_Timeline_Data.csv'
    
    # Process all JSON files
    all_data = process_all_json_files(json_dir)
    
    if all_data:
        # First sort by segment ID to keep waypointPath points with their parent segment
        # Then sort by start time within each segment group
        sorted_data = sorted(all_data, key=lambda x: (
            x.get('Segment ID', ''),
            parse_datetime(x.get('Start', ''))
        ))
